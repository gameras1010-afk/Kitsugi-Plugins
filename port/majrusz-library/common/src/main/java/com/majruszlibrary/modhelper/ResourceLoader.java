...
class ResourceLoader {
	...
	static class Server extends SimpleJsonResourceReloadListener {
		...
		protected void apply( Map< ResourceLocation, JsonElement > jsons, ResourceManager resourceManager, ProfilerFiller profilerFiller ) {
			for( String id : this.resources.keySet() ) {
				this.resources.get( id ).load( this.loader.gson, jsons.get( ResourceLocation.parse( id ) ) );
			}
		}
	}
	...
	static class Client {
		...
		public void loadResources( ResourceManager resourceManager ) {
			// Try listResources first
			var listed = resourceManager.listResources( this.loader.name, file->file.toString().endsWith( ".json" ) );
			for( ResourceLocation id : listed.keySet() ) {
				try {
					var res = this.resources.get( id.toString() );
					if( res != null ) res.load( this.loader.gson, listed.get( id ).openAsReader() );
				} catch( Exception exception ) {
					this.loader.helper.logError( exception.toString() );
				}
			}
			// Fallback: direct lookup for each registered resource not loaded by listResources
			for( var entry : this.resources.entrySet() ) {
				Resource< ? > res = entry.getValue();
				if( res.value != null ) continue; // already loaded
				ResourceLocation id = ResourceLocation.parse( entry.getKey() );
				try {
					var opt = resourceManager.getResource( id );
					if( opt.isPresent() ) {
						res.load( this.loader.gson, opt.get().openAsReader() );
					}
				} catch( Exception exception ) {
					this.loader.helper.logError( exception.toString() );
				}
