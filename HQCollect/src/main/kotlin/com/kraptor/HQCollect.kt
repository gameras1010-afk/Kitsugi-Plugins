...
class HQCollect : MainAPI() {
    ...
    private fun Element.toMainPageResult(): SearchResponse? {
        val title =
            this.selectFirst("h3 a")?.text() ?: this.selectFirst("img")?.attr("alt") ?: return null
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val recommendations =
            document.select("div.col-xs-6").mapNotNull { it.toRecommendationResult() }
    ...
    override suspend fun loadLinks(
        ...
        val videoSource = document.select("video [src], video[src], .wp-video a").attr("abs:src").ifBlank {
            document.select(".player iframe").attr("abs:src")
        }.takeIf { it.isNotBlank() }
        if (videoSource == null) return false
        val isDirectLink = videoSource.contains(".mp4") || videoSource.contains(".m3u8") || videoSource.contains("hqcollect.com")
        if (isDirectLink) {
            callback.invoke(
                newExtractorLink(
                    this.name,
            ...
            loadExtractor(videoSource, "$mainUrl/", subtitleCallback, callback)
        }
        return true
    }
}