"""
processor/pipeline/main.py
==========================
Ana çeviri pipeline'ı: process_and_replace_subtitle()
Dosya okuma → event filtreleme → batch çeviri → QA → yazma

Bağımlı modüller (processor/ içinden import edilir):
  - dedup, batch, style_detect, song_detect
  - song_translate, karaoke, tag_tools, text_helpers
"""
# ── Processor alt modüllerini içe al ──────────────────────────────────────
try:
    from processor.imports import *          # tüm try/except importlar
except ImportError:
    pass  # doğrudan çalıştırıldığında sys.path sorunu olabilir
...
try:
    from processor.batch import (
        collapse_animation_frames,
        broadcast_collapsed_frames,
...
try:
    from processor.style_detect import (
        is_sign_style_name, get_style_suffix_behavior,
        is_song_style_name, SONG_KEYWORDS,
        STYLE_SUFFIX_SKIP, STYLE_SUFFIX_FORCE_TRANSLATE,
        JP_CHARS, DRAWING_PATTERN, BRACKET_PATTERN,
        _COPYRIGHT_SKIP_RE, DELETE_KEYWORDS,
    )
except ImportError:
    pass
...
try:
    from processor.song_detect import (
        is_likely_song_by_content,
        is_likely_karaoke_syllable_by_content,
        _is_english_song_event, _get_song_type,
        _get_song_cache_path, _load_song_cache,
        _save_song_cache, _make_song_cache_key,
    )
except ImportError:
    pass
...
try:
    from processor.karaoke import (
        _preprocess_collapse_karaoke,
        _collapse_and_translate_karaoke,
    )
except ImportError:
    pass
...
try:
    from processor.text_helpers import (
        ...
        _is_english_content, is_romaji_text,
        clean_brackets, is_credit_line,
        parse_ass_time, format_ass_time,
        convert_srt_vtt_to_ass,
    )
except ImportError:
    pass
...
try:
    from processor.dedup import (
        _get_dedup_key,
        _check_termbase_compliance,
        _build_tb_lookup_from_prefs,
        _DEDUP_TAG_RE, _TB_SKIP_CATS,
    )
except ImportError:
    pass
...
def process_and_replace_subtitle(filepath, prefs, translator=None, output_path=None, time_offset=0.0):
    ...
    try:
        ...
        for _ev_item in _event_source:
            ...
            # [BUG-C FIX] Name/Actor alanında "Banner" veya "Credits" geçenler
            # bazı encoding araçları scroll efektini Name sütununa yazar.
            # Effect alanı boş ama Name alanı "Banner"/"Scroll"/"Credits" ise skip et.
            _ACTOR_BANNER_RE = re.compile(
                r'^(?:banner|scroll|credits?|staff|cast|credit_|rolling)',
                re.IGNORECASE
            )
            if actor_name and _ACTOR_BANNER_RE.match(actor_name.strip()):
                structured_events.append({
                    "line": line, "parts": parts,
                    "text": raw_text, "original_text": raw_text,
                    "type": "dialogue", "style": style_name,
                    "skip_translation": True, "reason": "actor_banner_scroll",
                    "actor": actor_name, "effect": effect_field,
                    "_pysubs2_ev": _ev_item.get('_ev'),
                })
                continue
            ...
            if not matches_skip_criteria:
                ...
                elif _style_beh is None and is_song_style_name(style_name):
                    # [BUG-B FIX] Sonek YOK (ör. "OP", "ED", "Opening") ama şarkı stili.
                    # İngilizce içerik varsa → SongPass'e yönlendir (romaji değil).
                    # Sonek yoksa _style_beh is None → buraya düşüyordu, SongPass atlanıyordu.
                    if _is_english_content(clean_text_no_tags) and not has_karaoke:
                        matches_skip_criteria = True
                        skip_reason = "song_en_phase2"
            ...
            if _romaji_skip_enabled and not matches_skip_criteria and clean_text_no_tags.strip():
                ...
                # [BUG-A FIX] OP/ED/Song stili + İngilizce içerik → romaji filtresine SOKMA.
                # Bu satırlar SongPass'e gitmeli ya da normal çeviriye gitmeli.
                # Romaji filtresi sadece JP/Romaji içerik için çalışır.
                _song_style_en_content = (
                    _is_song_st and
                    _style_beh2 != 'skip' and
                    _is_english_content(clean_text_no_tags)
                )
                _is_skip_style = (
                    (_is_song_st and not _song_style_en_content) or
                    (_style_beh2 == 'skip') or
                    _force_no_style
                )
            ...
            structured_events.append({
                ...
                "_pysubs2_ev": _ev_item.get('_ev'),
                # [Asama 1 — YENİ]
                # [SP2 FIX] pysubs2 aktif modda Name/Actor alanı _pysubs2_ev.name'de,
                # fallback modda ise parts[4]'te (ASS Format: Layer,Start,End,Style,Name,...)
                "actor": ((_ev_item.get('_ev').name.strip()
                           if _ev_item.get('_ev') and hasattr(_ev_item.get('_ev'), 'name') else None)
                          or (parts[4].strip() if len(parts) > 4 else "")),   # Karakter adı
        ...
        if prefs['translate'] and translator:
            ...
            for chunk in chunks:
                ...
                # 3. İlerleme ve Kayıt
                # [SP1 FIX] Segment-mod satırları pbar.update(1) ile zaten sayıldı.
                # Sadece segment-mod dışındaki satırları say — çift sayım önlenir.
                _seg_counted = sum(1 for _idx in indices_to_translate if _idx in seg_meta)
                _non_seg_count = len(chunk) - _seg_counted
                pbar.update(max(0, _non_seg_count))
