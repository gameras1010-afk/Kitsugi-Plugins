...
| **`SOSYAL-MODLAR.md`** | 🆕 🟢 **"Kim en son ne zaman girdi? Özel sohbet var mı?"** `/seen` + `/playtime` modları (ikisi de 1.21.1 NeoForge, server-only, bağımlılıksız). DM için: **vanilla `/msg` zaten var** — mod gerekmeyebilir. Web paneli: NeoForge'da istatistik modu **yok** (Plan/Player Statistics Fabric-only) → Crafty Controller kullan. Elenen 8 mod ve sebepleri |
