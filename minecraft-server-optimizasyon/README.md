...
| **`SOSYAL-MODLAR.md`** | 🆕 🟢 **"Kim en son ne zaman girdi? Web paneli? Özel sohbet?"** Modrinth API ile `versions:1.21.1`+`neoforge` facet filtresi kullanılarak doğrulandı. **Paradigm Essentials** üçünü de tek modda veriyor: `/msg`+`/r`+`@mention`+`/groupchat`, gömülü yerel web paneli (127.0.0.1), `/whois`, SQLite/MySQL kayıt — server-only, bağımlılıksız. Alternatifler: **Dash/ForgeDash** (güçlü panel ama Crafty ile çakışır), **Player Last Seen** (11 KB, sadece `/seen`). Elenen 9 mod + jar/SHA1 kanıtları |
